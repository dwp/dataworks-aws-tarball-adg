resource "aws_s3_bucket_object" "metadata_script" {
  ...
  key        = "component/tarball-adg/metadata.sh"
...
resource "aws_s3_bucket_object" "emr_setup_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/tarball-adg/emr-setup.sh"
  content = templatefile("${path.module}/bootstrap_actions/emr-setup.sh",
    {
      VERSION                         = local.tarball_adg_version[local.environment]
      ADG_LOG_LEVEL                   = local.tarball_adg_log_level[local.environment]
      ...
      acm_cert_arn                    = aws_acm_certificate.tarball_adg.arn
      ...
      cwa_log_group_name              = aws_cloudwatch_log_group.tarball_adg.name
      ...
      cwa_bootstrap_loggrp_name       = aws_cloudwatch_log_group.tarball_adg_cw_bootstrap_loggroup.name
      cwa_steps_loggrp_name           = aws_cloudwatch_log_group.tarball_adg_cw_steps_loggroup.name
      cwa_yarnspark_loggrp_name       = aws_cloudwatch_log_group.tarball_adg_cw_yarnspark_loggroup.name
  })
}

resource "aws_s3_bucket_object" "ssm_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/tarball-adg/start_ssm.sh"
  content = file("${path.module}/bootstrap_actions/start_ssm.sh")
}

resource "aws_s3_bucket_object" "installer_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/tarball-adg/installer.sh"
...
resource "aws_s3_bucket_object" "logging_script" {
  bucket  = data.terraform_remote_state.common.outputs.config_bucket.id
  key     = "component/tarball-adg/logging.sh"
  content = file("${path.module}/bootstrap_actions/logging.sh")
}

resource "aws_cloudwatch_log_group" "tarball_adg" {
  name              = local.cw_agent_log_group_name
  retention_in_days = 180
  tags              = local.common_tags
}

resource "aws_cloudwatch_log_group" "tarball_adg_cw_bootstrap_loggroup" {
  name              = local.cw_agent_bootstrap_loggrp_name
  retention_in_days = 180
  tags              = local.common_tags
}

resource "aws_cloudwatch_log_group" "tarball_adg_cw_steps_loggroup" {
  name              = local.cw_agent_steps_loggrp_name
  retention_in_days = 180
  tags              = local.common_tags
}

resource "aws_cloudwatch_log_group" "tarball_adg_cw_yarnspark_loggroup" {
...
resource "aws_s3_bucket_object" "cloudwatch_sh" {
  bucket = data.terraform_remote_state.common.outputs.config_bucket.id
  key    = "component/tarball-adg/cloudwatch.sh"
