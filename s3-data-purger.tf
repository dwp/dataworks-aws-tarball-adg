...
data "aws_iam_policy_document" "s3_data_purger_policy" {
  statement {
    ...
    resources = ["${data.terraform_remote_state.adg.outputs.published_bucket.arn}"]
  }
  statement {
    effect = "Allow"
    actions = [
    ...
    resources = [
      "${data.terraform_remote_state.adg.outputs.published_bucket.arn}/${local.adg_prefix[local.environment]}/*"
    ]
  }
  statement {
